...
use cso_demo::metadata::statistics::{
    Bucket, ColumnMetadata, ColumnStats, DistributionPolicyType, Histogram, RelationMetadata, RelationStats,
    StorageType,
};
...
fn md_cache() -> MdCache {
    ...
    // relation stats
    let relation_stats = RelationStats::new("x".to_string(), 9011.0, false, vec![column_stats_id.clone()]);
    ...
    let relation_md = RelationMetadata::new(
        ...
        false,
        StorageType::Heap,
        DistributionPolicyType::Hash,
        0,
        vec![8, 2],
        column_md,
        relation_stats_id.clone(),
    );
    let boxed_relation_md = Box::new(relation_md.clone()) as Box<dyn Metadata>;

    // column stats
    let buckets = vec![
        Bucket::new(Datum::I32(0), Datum::I32(1), 1.0, 2.0),
        Bucket::new(Datum::I32(1), Datum::I32(3), 3.0, 3.0),
    ];
    let histogram = Histogram::new(buckets);
    let column_stats = ColumnStats::new(1, 'x'.to_string(), Datum::I32(0), Datum::I32(1), 0.0, Some(histogram));
